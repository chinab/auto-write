...
<%@ page import="com.jekyll.common.framework.entity.PointMasterEntity"%>
...
<%
	...
	Map<String, PointMasterEntity> pointMasterMap = (Map<String, PointMasterEntity>) request.getAttribute("PointMasterMap");
				...
				<div class="admin_con">
					...
					<table class="tb2">
						...
						<tbody>
							...
							%>
							<%
								PointMasterEntity pointMasterEntity = null;
								int masterPoint = 0;
								int adminPoint = 0;
								int privatePoint = 0;
								int businessPoint = 0;
								int waiteressPoint = 0;
								
								for ( int ii = 0 ; ii < menuList.size() ; ii ++ ) {
									menu = (MenuDto)menuList.get(ii);
									pointMasterEntity = pointMasterMap.get(menu.getMenuid());
									
									masterPoint = pointMasterEntity.getMasterWrite();
									adminPoint = pointMasterEntity.getAdminWrite();
									privatePoint = pointMasterEntity.getPrivateWrite();
									businessPoint = pointMasterEntity.getBusinessWrite();
									waiteressPoint = pointMasterEntity.getWaiteressWrite();
							%>
			 				<tr>
								<td><%= ii+1 %></td>
								<td class="subject">
								...
								<td>
									<%=PointMasterEntity.writePoint(masterPoint)%>
								</td>
								<td>
									<%=PointMasterEntity.writePoint(adminPoint)%>
								</td>
								<td>
									<%=PointMasterEntity.writePoint(privatePoint)%>
								</td>
								<td>
									<%=PointMasterEntity.writePoint(businessPoint)%>
								</td>
								<td>
									<%=PointMasterEntity.writePoint(waiteressPoint)%>
								</td>
